...
server.get('', function (req, res) {
        // Adaptation du message en fonction du device du client
        ...
        if (ipInfo.city == undefined) {
            ipInfo.city = "Impossible de localiser le client.".red
        }

        // LOG Date
        console.log('>>>'.blue + date.getHours() + 'h' + date.getMinutes());
        ...
        if (req.session.name == undefined) {
            ...
            console.log('Nouveau visiteur ! On l\'appellera "'.green + req.session.name.yellow + '".'.green);
        } else {
            req.session.time += 1;
            req.session.compteur += 1;

            console.log('Nouvelle connexion de ' + req.session.name.green + '. Connexion n°' + req.session.time);
        }

        console.log('CITY : '.yellow + ipInfo.city);
        console.log('DEVICE : '.yellow + req.device.type);

        if (ipInfo.city == "Impossible de localiser le client".red) {
            ...
            http.get('http://api.openweathermap.org/data/2.5/weather?q=' + undefined + ',fr&appid=36c2a73dd80e36cc1c36eab251bbf29e', (resp) => {
                ...
                resp.on('end', () => {
                    var CODE = JSON.parse(data).cod.toString().green;
                    ...
                    // Calcul de la couleur d'arrière plan
                    {
                    ...
                    if (req.session.compteur == 1) {
                        if (meteo >= 22) {
                            var premierMsg = msg1_chaud;
                        } else {
                            var premierMsg = msg1_froid;
                            msg_device = "";
                            console.log(">>>".blue + " Retour à zéro pour " + +req.session.name.yellow + "... (cause: 'trop froid')\n".red);
                            req.session.compteur = 0;
                        }
                    } else if (req.session.compteur < 5) {
